...
// INICIO DAS CONSULTAS NA TELA DE USUÁRIO
    ...
    {
        $this->db->where('id', $idPropriedade);
        $res = $this->db->delete('tb_propriedades');
   ...
   //     $this->db->join('tbl_roles as Role', 'Role.roleId = Usuarios.roleId','left');
        ...
        $likeCriteria2 = "(Parcelas.id_acesso = ".$this->session->userdata('userId')."
    ...
    {
        $this->db->where('id', $idParcela);
        $res = $this->db->delete('tb_parcelas');
    ...
    function carregaInfoAnimal($IdAnimal)
    {
        ...
        $this->db->join('tb_fauna_genero as faunaGenero', 'faunaGenero.id = Rl.id_genero','left');
        $this->db->join('tb_fauna_especie as faunaEspecie', 'faunaEspecie.id = Rl.id_especie','left'); 
...
// FIM DAS CONSULTAS NA TELA DE PERMISSAO
   ...
   //     $this->db->join('tbl_roles as Role', 'Role.roleId = Usuarios.roleId','left');
        ...
        $likeCriteria2 = "(ArvoresVivas.id_acesso = ".$this->session->userdata('userId')."
        OR Perfil.st_admin = 'S')";
        $this->db->where($likeCriteria2);
    ...
    {
        $this->db->where('id', $IdArvoreViva);
        $res = $this->db->delete('tb_arvores_vivas');
    ...
    function apagaRlFloraFamiliaGeneroEspecie($IdRlFloraFamiliaGeneroEspecie)
    {
        ...
        $res = $this->db->delete('rl_flora_familia_genero_especie');
   ...
   //     $this->db->join('tbl_roles as Role', 'Role.roleId = Usuarios.roleId','left');
    ...
    function adicionaAnimal($infoAnimal)
    {
        ...
        `st_registro_ativo`) VALUES ({$infoAnimal['id']}, {$infoAnimal['id_parcela']}, '{$infoAnimal['id_acesso']}', '{$infoAnimal['latitude_gms']}', 
        '{$infoAnimal['longitude_gms']}', {$infoAnimal['id_tipo_observacao']}, {$infoAnimal['id_classificacao']}, '{$infoAnimal['id_grau_protecao']}', 
        '{$infoAnimal['descricao']}', '{$infoAnimal['latitude_gd']}', '{$infoAnimal['longitude_gd']}', '{$infoAnimal['st_registro_ativo']}')";
        $query = $this->db->query($sql);
        $this->db->stop_cache();
        $this->db->flush_cache();
        return $this->db->affected_rows();
    ...
    function apagaRlFaunaFamiliaGeneroEspecie($IdRlFaunaFamiliaGeneroEspecie)
    {
        ...
        $res = $this->db->delete('rl_fauna_familia_genero_especie');
   ...
   //     $this->db->join('tbl_roles as Role', 'Role.roleId = Usuarios.roleId','left');
        ...
        $likeCriteria2 = "(Epifitas.id_acesso = ".$this->session->userdata('userId')."
        OR Perfil.st_admin = 'S')";
        $this->db->where($likeCriteria2);
    ...
    {
        $this->db->where('id', $IdEpifita);
        $res = $this->db->delete('tb_epifitas');
    ...
    function listaHidrologia($searchText = '', $page, $segment)
    {
        ...
        $likeCriteria2 = "(Hidrologia.id_acesso = ".$this->session->userdata('userId')."
        OR Perfil.st_admin = 'S')";
        $this->db->where($likeCriteria2);
    ...
    {
        $this->db->where('id', $IdHidrologia);
        $res = $this->db->delete('tb_hidrologia');
    ...
    function apagaRlEpifitaFamiliaGeneroEspecie($IdRlEpifitaFamiliaGeneroEspecie)
    {
        ...
        $res = $this->db->delete('rl_epifitas_familia_genero_especie');
    ...
    {
        $this->db->select('(CASE WHEN isnull(`nu_parcela`) THEN 1 WHEN `nu_parcela` <> null or `nu_parcela` <> 0 THEN max(nu_parcela)+1 END) as next_nu_parcela');        
        $this->db->from('tb_parcelas as Parcelas');
